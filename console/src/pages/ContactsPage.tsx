...
import {
  ...
  JsonViewer,
} from "../components/contacts/ContactColumnsSelector";
...
export function ContactsPage() {
  ...
  const { workspaceId } = useParams({
    from: "/console/workspace/$workspaceId/contacts",
  });
  ...
  const [contactToEdit, setContactToEdit] = React.useState<Contact | null>(
    null,
  );
  // Export modal state
  ...
  React.useEffect(() => {
    ...
    const handler = (e: BeforeUnloadEvent) => {
      e.preventDefault();
      e.returnValue =
        "A bulk action is in progress. Leaving now will abort it.";
    };
  ...
  const { data: segmentsData } = useQuery({
    ...
    queryFn: () =>
      listSegments({ workspace_id: workspaceId, with_count: true }),
    ...
    onMutate: async (deletedEmail) => {
      ...
      queryClient.setQueryData(
        ...
        (old: { contacts: Contact[]; next_cursor?: string } | undefined) => {
          ...
          return {
            ...old,
            contacts: old.contacts.filter((c) => c.email !== deletedEmail),
          };
        },
      );
      return { previous };
    ...
    queryFn: async ({ signal }) => {
      ...
      while (allContacts.length < targetCount) {
        ...
        const batchSize = Math.min(
          API_MAX_LIMIT,
          targetCount - allContacts.length,
        );
  ...
  const columns: ColumnsType<Contact> = [
        ...
        <Space orientation="vertical" size={2}>
          {record.contact_lists?.map(
            (list: {
              ...
              const listData = listsData?.lists?.find(
                (l) => l.id === list.list_id,
              );
              ...
              const tooltipTitle = (
                <>
                  ...
                  <div>
                    {t`Subscribed on:`} {creationDate}
                  </div>
                  <div>
                    <small>
                      {t`Timezone:`} {workspaceTimezone}
                    </small>
                  </div>
                </>
              );
        ...
        <Space orientation="vertical" size={2}>
          {record.contact_segments?.map(
            (segment: {
              ...
              const segmentData = segmentsData?.segments?.find(
                (s) => s.id === segment.segment_id,
              );
              ...
              const tooltipTitle = (
                <>
                  ...
                  <div>
                    {t`Matched on:`} {matchedDate}
                  </div>
                  {segment.version && (
                    <div>
                      {t`Version:`} {segment.version}
                    </div>
                  )}
                  <div>
                    <small>
    ...
    },
    {
      ...
      key: "custom_datetime_1",
      render: (_: unknown, record: Contact) =>
        record.custom_datetime_1
          ? new Date(record.custom_datetime_1).toLocaleDateString()
          : "-",
    ...
    },
    {
      ...
      key: "custom_datetime_2",
      render: (_: unknown, record: Contact) =>
        record.custom_datetime_2
          ? new Date(record.custom_datetime_2).toLocaleDateString()
          : "-",
    ...
    },
    {
      ...
      key: "custom_datetime_3",
      render: (_: unknown, record: Contact) =>
        record.custom_datetime_3
          ? new Date(record.custom_datetime_3).toLocaleDateString()
          : "-",
    ...
    },
    {
      ...
      key: "custom_datetime_4",
      render: (_: unknown, record: Contact) =>
        record.custom_datetime_4
          ? new Date(record.custom_datetime_4).toLocaleDateString()
          : "-",
    ...
    },
    {
      ...
      key: "custom_datetime_5",
      render: (_: unknown, record: Contact) =>
        record.custom_datetime_5
          ? new Date(record.custom_datetime_5).toLocaleDateString()
          : "-",
              ...
              items: [
                {
                  ...
                  disabled: !permissions?.contacts?.write,
                  onClick: () =>
                    openImportDrawer(workspaceId, listsData?.lists || [], true),
                },
                {
                  key: "export",
                  label: t`Export`,
      ...
      <Table
        ...
        onRow={(record) => ({
          onClick: (e) => {
            ...
            if (
              target.closest(
                ".ant-table-selection-column, .contacts-actions-col",
              )
            )
              return;
            setSelectedEmails((prev) =>
          ...
          style: permissions?.contacts?.write
            ? { cursor: "pointer" }
            : undefined,
        })}
        locale={{
